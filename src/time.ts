...
export function isTimeBalanced(time: string, type: TimeTypes): boolean {
  switch (type) {
    case TimeTypes.ABSOLUTE: {
      const balancedTime = parseDoyOrYmdTime(
        getDoyTime(new Date(getUnixEpochTime(time)))
      ) as ParsedDoyString;
...
export function parseDurationString(
  ...
  const durationTime = parseDoyOrYmdTime(
    durationString
...
export function getBalancedDuration(time: string): string {
  ...
  const balancedTime = getDoyTime(
    new Date(getUnixEpochTime(convertDurationToDoy(duration)))
  );
 ...
 */
export function convertDurationStringToInterval(
  durationString: string
): string | never {
  const parsedDuration = parseDurationString(
    convertUsToDurationString(convertDurationStringToUs(durationString))
  );
 ...
 */
export function convertDurationStringToUs(
  durationString: string
): number | never {
 ...
 */
export function convertUsToDurationString(
  durationUs: number,
 ...
 */
export function getIntervalUnixEpochTime(
  startTimeMs: number,
  endTimeMs: number
): string {
...
function parseDOYDurationTime(doyTime: string): ParsedDurationString | null {
  ...
  const matches = RELATIVE_TIME.exec(doyTime);
  if (matches !== null) {
    if (matches) {
      const {
        groups: {
